  ...
  <div class="flow-container">
    <div class="toolbar">
      ...
      <div 
        class="node-item" 
        draggable="true"
        @dragstart="onDragStart('custom-node')"
      >
        自定义节点
      </div>
  ...
  methods: {
    initGraph() {
      ...
      Graph.registerVueComponent(
        'custom-node', 
        {
          template: `<custom-node :node-data="nodeData" />`,
          components: { CustomNode }
        }, 
        true
      )
      ...
      this.graph.on('edge:connected', () => console.log('Edge connected'))
    },
    ...
    initDnd() {
      this.dnd = new Addon.Dnd({
        ...
        validateNode(droppingNode) {
          return droppingNode.shape === 'vue-shape'
        }
      })
    },
    ...
    editNode(node) {
      ...
      if (targetNode) {
        ...
        if (title !== null) {
          ...
          // 更新节点内容
          const itemsStr = prompt('请输入节点内容(用逗号分隔)', data.items.join(','))
          if (itemsStr !== null) {
            const items = itemsStr.split(',').map(item => item.trim()).filter(item => item)
            targetNode.setData({ ...data, title, items })
          }
        }
      }
      this.hideContextMenu()
